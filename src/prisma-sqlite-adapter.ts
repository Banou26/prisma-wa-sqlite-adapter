...
import type { Database, Statement } from './wa-sqlite-better-sqlite3'
...
class SQLiteQueryable implements SqlQueryable {
...
class SQLiteTransaction extends SQLiteQueryable implements Transaction {
  constructor(
    db: Database,
...
// Main adapter
class SQLiteAdapter extends SQLiteQueryable implements SqlDriverAdapter {
  readonly tags = {
    error: '[prisma:error]',
    warn: '[prisma:warn]',
    info: '[prisma:info]',
  ...
  async startTransaction(isolationLevel?: IsolationLevel): Promise<Transaction> {
    ...
    try {
      ...
      this.db.exec('BEGIN')
      return new SQLiteTransaction(this.db, options, release)
    } catch (error) {
      this.handleError(error)
    }
  }
...
// Factory function
export function createAdapter(db: Database): SqlDriverAdapter {
